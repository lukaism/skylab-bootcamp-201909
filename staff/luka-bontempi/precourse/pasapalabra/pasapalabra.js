...
function pasapalabra(){
    let index=0
    user=prompt('Welcome! Insert your user name:')
    if (confirm('Ready to start, ' + user +'?')){
        while (completed(questions)){
            answer=prompt(questions[index].question)
            if (answer === questions[index].answer){
                alert('Correcto!')
                questions[index].status=1
                index=nextIndex(questions,index)
                ...
                alert('Incorrecto! ' + questions[index].answer)
                questions[index].status=-1
                index=nextIndex(questions,index)
            } 
        }
        alert('Your score is: ' + points(questions) + ' points!')
    }
    alert('Ciao!')
}
