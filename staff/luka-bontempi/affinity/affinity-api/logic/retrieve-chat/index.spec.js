...
const { errors: { NotFoundError, ContentError } } = require('affinity-util')
const { ObjectId, database, models: { User, Chat, Message } } = require('affinity-data')

describe('logic - retrieve-complete-user', () => {
    before(() => database.connect(TEST_DB_URL))

    let id1, id2, id3, chatId, name, surname, email, username, genderId, description, geometric, password, birthdate, message1, message2, messages
    ...
    beforeEach(async () => {
        ...
        const user1 = await User.create({ name, surname, email, username, genderId, description, geometric, password, birthdate})
        id1 = user1.id

        const user2 = await User.create({ name, surname, email, username: username1, genderId, description, geometric, password, birthdate})
        id2 = user2.id

        const user3 = await User.create({ name, surname, email, username: username2, genderId, description, geometric, password, birthdate})
        id3 = user3.id
