...
describe('logic - check-connection', () => {
    ...
    it('should succeed on correct user id', async () => {
        ...
        expect(usera.connections).toBeOfType('array')
        expect(userb.connections).toBeOfType('array')
        expect(usera.connections).toEqual([id1])
        expect(userb.connections).toEqual([id])
        expect(usera.lastAccess).toBeDefined()
        expect(usera.lastAccess).toBeOfType(Date)
        expect(userb.lastAccess).toBeDefined()
        expect(userb.lastAccess).toBeOfType(Date)
    })
    ...
    it('should not update connections when user not in user1s aproved list', async () => {
        ...
        expect(usera.connections).toBeOfType('array')
        expect(userb.connections).toBeOfType('array')
        expect(usera.connections).toEqual([])
        expect(userb.connections).toEqual([])
